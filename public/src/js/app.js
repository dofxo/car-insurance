...
const calculateButton = document.querySelector("#calculate-button-wrapper");

// call the calculate method in the calculate class
const insuranceCheckBoxes = document.querySelectorAll(
  ".insurance-type-check-box"
);
calculateButton.addEventListener("click", () => {
  // car model value
  const carModelNameValue = document.querySelector(
    "#car-model-box > span"
  ).textContent;
  // car registration value
  const carRegistrationYearValue = document.querySelector(
    "#car-registration-year-box > span"
  ).textContent;
  // insuranceType
  let insuranceTypeValue;
  // choosing the checked insuranceType
  insuranceCheckBoxes.forEach((item) => {
    if (item.classList.contains("checked")) {
      insuranceTypeValue = item.nextElementSibling.textContent;
    }
  });
  // gathering the inforamtions for calculating the insurance
  new CalculateInsurance(
    carModelNameValue,
    carRegistrationYearValue,
    insuranceTypeValue
  );
});
